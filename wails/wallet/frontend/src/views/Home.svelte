...
<main>
  <div class="balance-container">
    ...
    <div class="balance-buttons-container">
      <button id="send-crypto-button" on:click={sendCrypto}>Send</button>
        ...
        {#each assetsArray as asset, index (index)}
          ...
          <div class="account-details">
            <div class="account-dropdown" bind:this={dropdownRef}>
              <button on:click={() => listTokenAccounts()} class="account-dropdown-button"
                >{asset.accounts[asset.selectedAccount].slice(0, 4) +
                  '...' +
                  asset.accounts[asset.selectedAccount].slice(-3)}</button
              >
              {#if showDropdown}
                <ul class="account-dropdown-content">
                  {#each Object.entries(asset.accounts) as [key, account], index (index)}
                    <li on:click={() => selectAccount(asset, key)} tabindex="0">
