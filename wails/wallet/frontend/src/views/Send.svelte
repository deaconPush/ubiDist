<script lang="ts">
  ...
  function validateAddress(): void {
    ...
    const addressValidationLabel = document.getElementById(
    ...
    if (address.length !== 0) {
      ValidateAddress(address, token).then((ok: boolean) => {
        if (!ok) {
  ...
  function validateAmount(e: Event): void {
    ...
    const amountValidationLabel = document.getElementById(
      'amount-validation-label'
    ) as HTMLParagraphElement;
    const continueTransactionButton = document.getElementById(
  ...
  function sendTransaction(e: Event): void {
    ...
    SendTransaction(
      ...
      password,
      sendingAddress,
      confirmedTransactionAmount.toString(),
      currentAsset.selectedAccount
    )
      .then((ok: boolean) => {
        if (ok) {
...
<main>
  ...
  {:else if currentComponent === 'Set Token Amount'}
    ...
    <button id="continue-transaction-button" disabled="true" on:click={confirmAmount}
      >Continue</button
    >
    ...
    {#if showPasswordModal === true}
      <div class="overlay">
        <form class="form-container">
          <div class="input-group">
            <label for="password"
              >To proceed with the transaction you need to sign it with your password.</label
            >
